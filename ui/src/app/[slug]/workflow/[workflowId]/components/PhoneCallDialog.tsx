...
export const PhoneCallDialog = ({
    ...
    const fetchPreferences = useCallback(async () => {
        ...
        if (result.error) {
            throw new Error(detailFromError(result.error, t('phone.errors.loadPreferences')));
        }
        return result.data || {};
    }, [t]);
    ...
    useEffect(() => {
        ...
        const loadPreferences = async () => {
            try {
                ...
                applyPreferences({});
                setPreferencesLoaded(false);
                setCallError(err instanceof Error ? err.message : t('phone.errors.loadPreferences'));
            }
        };
    ...
    const savePhoneNumberPreference = async () => {
        ...
        if (result.error) {
            throw new Error(detailFromError(result.error, t('phone.errors.savePreferences')));
        }
        if (!result.data) {
            throw new Error(t('phone.errors.savePreferences'));
        }
    ...
    const handleStartCall = async () => {
        ...
        try {
            ...
            if (response.error) {
                let errMsg = t('phone.errors.initiateCall');
                ...
                const msg = response.data && (response.data as unknown as { message: string }).message || t('phone.callInitiatedSuccess');
                setCallSuccessMsg(typeof msg === "string" ? msg : JSON.stringify(msg));
            }
        } catch (err: unknown) {
            setCallError(err instanceof Error ? err.message : t('phone.errors.initiateCall'));
        } finally {
            setCallLoading(false);
        }
    };
    ...
    const renderConfigurationNeeded = () => (
        <>
            <DialogHeader>
                <DialogTitle>{t('phone.configureTelephony.title')}</DialogTitle>
                <DialogDescription>
                    {t('phone.configureTelephony.description')}
                </DialogDescription>
            </DialogHeader>
            <DialogFooter>
                <Button variant="ghost" onClick={() => onOpenChange(false)}>
                    {t('phone.configureTelephony.doItLater')}
                </Button>
                <Button onClick={handleConfigureContinue}>
                    {t('phone.configureTelephony.continue')}
                </Button>
            </DialogFooter>
        </>
    );
    ...
    const renderPhoneCallForm = () => (
        <>
            <DialogHeader>
                <DialogTitle>{t('phone.title')}</DialogTitle>
                <DialogDescription>
                    {t('phone.form.description')}
                </DialogDescription>
            </DialogHeader>
            {telephonyConfigs.length > 0 && (
                <div className="flex flex-col gap-1.5">
                    <Label htmlFor="telephony-config">{t('phone.form.telephonyConfigLabel')}</Label>
                    <Select value={selectedConfigId} onValueChange={setSelectedConfigId}>
                        <SelectTrigger id="telephony-config" className="w-full">
                            <SelectValue placeholder={t('phone.form.selectConfigPlaceholder')} />
                        </SelectTrigger>
                        <SelectContent>
                            {telephonyConfigs.map((config) => (
                                <SelectItem key={config.id} value={String(config.id)}>
                                    {config.name} ({config.provider})
                                    {config.is_default_outbound ? ` — ${t('phone.form.defaultSuffix')}` : ""}
                                </SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
            ...
            {selectedConfigId && (
                <div className="flex flex-col gap-1.5">
                    <Label htmlFor="from-phone-number">{t('phone.form.callerIdLabel')}</Label>
                    {loadingPhoneNumbers ? (
                        <div className="flex items-center text-sm text-muted-foreground">
                            <Loader2 className="h-4 w-4 animate-spin me-2" />
                            {t('phone.form.loadingPhoneNumbers')}
                        </div>
                        ...
                        >
                            <SelectTrigger id="from-phone-number" className="w-full">
                                <SelectValue placeholder={t('phone.form.selectPhoneNumberPlaceholder')} />
                            </SelectTrigger>
                            <SelectContent>
                                {fromPhoneNumbers.map((phone) => (
                                    <SelectItem key={phone.id} value={String(phone.id)}>
                                        ...
                                        {phone.is_default_caller_id ? ` — ${t('phone.form.defaultSuffix')}` : ""}
                                    </SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
            ...
            >
                {sipMode ? t('phone.form.usePhoneNumberInstead') : t('phone.form.useSipEndpointInstead')}
            </button>
            <DialogFooter className="flex-col sm:flex-row gap-2">
                ...
                >
                    {t('phone.form.configureTelephony')}
                </Button>
                <div className="flex gap-2 flex-1 justify-end">
                    <DialogClose asChild>
                        <Button variant="outline">{t('phone.form.cancel')}</Button>
                    </DialogClose>
                    {!callSuccessMsg ? (
                        <Button
                        ...
                        >
                            {callLoading ? t('phone.form.calling') : t('phone.form.startCall')}
                        </Button>
                    ) : (
                        <>
                            <Button variant="outline" onClick={() => { setCallSuccessMsg(null); setCallError(null); }}>
                                {t('phone.form.callAgain')}
                            </Button>
